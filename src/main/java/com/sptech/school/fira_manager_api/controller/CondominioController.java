...
public class CondominioController {
    ...
    @PostMapping
    public ResponseEntity<CondominioResponse> adicionarCondominio(@Valid @RequestBody CondominioRequest dto) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Condominio atualizado com sucesso",
                    content = @Content(schema = @Schema(implementation = CondominioResponse.class))),
            @ApiResponse(responseCode = "400", description = "Requisição inválida", content = @Content),
            @ApiResponse(responseCode = "404", description = "Condominio não encontrado", content = @Content)
    })
    @PutMapping("/{id}")
    public ResponseEntity<CondominioResponse> atualizarCondominio(@PathVariable Long id, @Valid @RequestBody CondominioRequest dto) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "Condominio deletado com sucesso", content = @Content),
            @ApiResponse(responseCode = "404", description = "Condominio não encontrado", content = @Content)
    })
    @DeleteMapping("/{id}")
